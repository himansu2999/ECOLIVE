...
import Showwmf from '../../components/UI/Showwmf/Showwmf';
import { withRouter } from 'react-router-dom';
import { authUserContext, withAuthentication } from '../../components/Session'
import { withFirebase } from '../../components/Firebase';
import Calendar from '../../components/CalendarComponent/Calendar/Index'
...
class Appointments extends Component {
                ...
                if(authUser){
                    ...
                    axios.get(`/users/${authUser.uid}/activities.json`).then(res => {
                        ...
                        for (let key in res.data) {
                            if((res.data!=="not set")&&(res.data[key].doctorId!=="not set")){
    ...
    showFormHandler = (request) => {
        ...
        // const Weekly = database.ref('Weekly');
        var reqData
        var firstTableData
        var textAreasData
        if(request.weekly!==undefined){
        this.setState(
            {
                reqData: request.weekly.requestData,
                    firstTableData: request.weekly.firstTableData,
                    textAreasData: request.weekly.textAreasData,
                    showForm : true
            }
        )
        }
    ...
    render() {
        ...
        if (!this.state.loading) {
            ...
            calendar =(<div className={classes.Calendar}>
                <h2 style={{color: 'blue'}}>Calendar</h2>
        ...
        if(this.state.showForm){
            ...
            {<Showwmf
                requestData={this.state.reqData}
